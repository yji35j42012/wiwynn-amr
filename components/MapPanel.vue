<template>
	<div class="mappanel">
		<div id="mappanel_box" class="mappanel_box">
			...
			<div class="mappanel_box_row" style="top: 100px;left: 50px">
				<div v-for="(index) in 5" :key="index" class="side"
					:style="'transform: rotateX(-30deg) rotateY(290deg) translateX(0px) translateY(' + index * 0 + 'px);left:' + (index) * 50 + 'px;top:' + index * -20 + 'px;'">
					<div class="side_box _top"></div>
					<div class="side_box _left"></div>
			...
			<div class="mappanel_box_row" style="top: 200px;left: 100px">
				<div v-for="(index) in 5" :key="index" class="side"
					:style="'transform: rotateX(-30deg) rotateY(290deg) translateX(0px) translateY(' + index * 0 + 'px);left:' + (index) * 50 + 'px;top:' + index * -20 + 'px;'">
					<div class="side_box _top"></div>
					<div class="side_box _left"></div>
...
export default {
	...
	mounted() {
		let mappanel_box=document.getElementById('mappanel_box');
		let canvas=document.getElementById('mycanvas');
		canvas.style=`width:${ mappanel_box.offsetWidth }px; height:${ mappanel_box.offsetHeight }px;`
		console.log('w', mappanel_box.offsetWidth);
		console.log('w', mappanel_box.offsetHeight);
	},
	methods: {
		doDraw() {
			let canvas=document.getElementById('mycanvas');
			let a=setInterval(() => {
				...
				if (!canvas) {
					...
					this.pointData.forEach((item) => {
						...
						context.strokeSytle='rgb(20,20,235)';
						context.beginPath();
